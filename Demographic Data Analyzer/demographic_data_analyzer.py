...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(
        (len(df[(df["hours-per-week"] == 1) & (df.salary == ">50K")]) / num_min_workers)
        * 100,
        1,
    )
    ...
    for country_name in set(df["native-country"].values):
        country_percentage = round(
            (
                len(df[(df["native-country"] == country_name) & (df.salary == ">50K")])
                / df["native-country"].values.tolist().count(country_name)
            )
            * 100,
            1,
        )
        if (
            max(highest_earning_country_and_percentage[1], country_percentage)
            != highest_earning_country_and_percentage[1]
        ):
